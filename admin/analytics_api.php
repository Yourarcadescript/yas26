...
/*
 * This code was modified by Fernando Hillebrand <fernando@hillebrand.com.ar> to work with api version 2.4
 * 
 * This class is based on the code developed by Chris Hope <chris@electrictoolbox.com> and made available
...
//-------------------------------------------------------------------------------------------------
	...
	*/
	public function getAccountsTableIdFromName($name){
	//---------------------------------------------------------------------------------------------
		...
		foreach($entries as $entry) {
			...
			foreach($properties as $property) {
				switch($property->getAttribute('name')) {
					case 'ga:accountId':
						$this->accounts[$title]['accountId'] = $property->getAttribute('value');	break;
					case 'ga:accountName':
						$this->accounts[$title]['accountName'] = $property->getAttribute('value');	break;
					case 'ga:webPropertyId':
						$this->accounts[$title]['webPropertyId'] = $property->getAttribute('value');	break;
					case 'ga:profileId':
						$this->accounts[$title]['profileId'] = $property->getAttribute('value');	break;
					case 'dxp:tableId':
						$this->accounts[$title]['tableId'] = $property->getAttribute('value');	break;
				}
			}
		}
		ksort($this->accounts);
	...
	//---------------------------------------------------------------------------------------------
		...
		$url = "https://www.googleapis.com/analytics/v2.4/data?ids=$id&dimensions=$dimension&metrics=$metric&sort=$sort&start-date=$start&end-date=$end&max-results=$max_results&start-index=$start_index";
		if($filters) {
