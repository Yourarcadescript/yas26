...
include_once("mgf_functions.php");
...
<div class="top-bar">
<h1>Install Mochi Game Feed</h1>
<div class="select-bar">
<center>
<form id="mgffeed" name="mgffeed" method="get" action="index.php">
<input type="hidden" name="act" value="managemgffeed">
...
$result = yasDB_select("SELECT COUNT(id) FROM `mgffeed`");
...
$sql = 'SELECT count(id) FROM mgffeed WHERE' . $sql_category . $sql_title . $sql_description . $sql_rating . $sql_ads . ' installed = "0"';
...
//*************************************************************************************************************************************************
$sql = 'SELECT * FROM mgffeed WHERE' . $sql_category .$sql_title. $sql_description . $sql_rating . $sql_ads . ' installed = "0" ORDER BY id DESC '.$limit;
$query = yasDB_select($sql,false);
if($query->num_rows == 0) {
...
if ($pageno == 1) {
   ...
   echo ' <a href="index.php?act=managemgffeed&page=1&category='.$category.'&description='.$description.'&title='.$title.'&filter=Filter games">FIRST</a>';
   $prevpage = $pageno-1;
   echo ' <a href="index.php?act=managemgffeed&page=' . $prevpage . '&category='.$category.'&description='.$description.'&title='.$title.'&filter=Filter games">PREV</a>';
}
...
if ($pageno == $lastpage) {
   ...
   $nextpage = $pageno+1;
   echo ' <a href="index.php?act=managemgffeed&page=' . $nextpage . '&category='.$category.'&description='.$description.'&title='.$title.'&filter=Filter games">NEXT</a>';
   echo ' <a href="index.php?act=managemgffeed&page=' . $lastpage . '&category='.$category.'&description='.$description.'&title='.$title.'&filter=Filter games">LAST</a>';
}
echo '</div>';
if (isset($_GET['install'])) {
  if ($_GET['install'] == 'Install') {
    install_mgfgame($_GET['gameid']) or die("Game did not install successfully");
