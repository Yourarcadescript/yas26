...
function deleteChecked(chked) {
	...
	if (response) {
		$.post('deleteall.php', {'checkbox[]': boxArray, 'sessionid': '<?php echo session_id() ?>' }, function(data){ location.reload(true); });
	}
}
</script>
...
if(!empty($_GET['edit'])) {
	...
	if($query->num_rows == 0) {
		...
		<div class="table">
			...
			<td class="last"><textarea name="description" id="edit_description" cols="35" rows="7" style="width:275px;"/><?php echo $row['description'];?></textarea></td></td>
			</tr>
            <tr>
