...
$cache = RayCache::getInstance('mostplayed'.$page, null, array('prefix' => $setting['theme'].'-'.$setting['skin'].'_', 'path' => 'cache/', 'expire' => $setting['cachelife']));
$data = $cache->read('mostplayed');
if ($data) {
	echo $data;
} else {
	$cache->start_caching();
	...
	<!-- begin center -->
	<div id="center">
	<div class="container_box1"><div id="headergames2">Most Played</div>
	<?php
	...
	if ($query->num_rows == 0) {
		...
		$query = yasDB_select("SELECT id, title, thumbnail, description, plays FROM games ORDER BY `plays` desc LIMIT " . $pageurl->start . ", " . $pageurl->limit);
	...
	<!-- end of mostplayed -->
	<?php
	$cache->write('mostplayed'.$page);
	$cache->stop_caching();
	echo $cache->read('mostplayed'.$page);
}
?>